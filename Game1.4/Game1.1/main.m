...
for t = 0:timeStep:endTime
    if flag == 1
        ...
        if red_striker.position(1) >= 7.5
            ...
            if ball.position(1) >= 10.5
                fprintf("ball in")
                pause(3);
                set(ballPos, 'XData', MapLength/2, 'YData',4);
                set(red_strikerPos, 'XData', 4.5, 'YData',MapHeight/2);
                set(red_defender1Pos, 'XData', 3, 'YData',MapHeight/3);
                set(red_defender2Pos, 'XData', 3, 'YData',2*MapHeight/3);
                set(red_goalkeeperPos, 'XData', 1, 'YData',MapHeight/2);
    
                set(blue_strikerPos, 'XData', MapLength-4.5, 'YData',MapHeight/2);
                set(blue_defender1Pos, 'XData', MapLength-3, 'YData',MapHeight/3);
                set(blue_defender2Pos, 'XData', MapLength-3, 'YData',2*MapHeight/3);
                set(blue_goalkeeperPos, 'XData', MapLength-1, 'YData',MapHeight/2);
    
                red_striker.position = [4.5, MapHeight/2];
                red_defender1.position = [3, MapHeight/3];
                red_defender2.position = [3, 2*MapHeight/3];
                red_goalkeeper.position = [1, MapHeight/2];
    
                blue_striker.position = [MapLength-4.5, MapHeight/2];
                blue_defender1.position = [MapLength-3, MapHeight/3];
                blue_defender2.position = [MapLength-3, 2*MapHeight/3];
                blue_goalkeeper.position = [MapLength-1, MapHeight/2];
                flag = 0;
                pause(1);
            end
        end
    ...
    if flag == 0
        ...
        x1 = 2.5*rand(1)-1;
        y1 = 2.5*rand(1)-1;
        set(red_strikerPos, 'XData', leftStriker_pos(1)+timeStep*x1, 'YData',leftStriker_pos(2)+timeStep*y1);
        red_striker.position = [leftStriker_pos(1)+timeStep*x1, leftStriker_pos(2)+timeStep*y1];
        
        x2 = 3*rand(1);
        y2 = 2.5*rand(1)-1;
        if right_defender1_pos(1) <= 3
            x2=0.02*(rand(1)-1);
        end
        set(blue_defender1Pos, 'XData', right_defender1_pos(1)-timeStep*x2, 'YData',right_defender1_pos(2)+timeStep*y2);
        blue_defender1.position = [right_defender1_pos(1)-timeStep*x2, right_defender1_pos(2)+timeStep*y2];
        ...
        y6 = 2*rand(1)-1;
        set(red_goalkeeperPos, 'XData', left_goalkeeper_pos(1), 'YData',left_goalkeeper_pos(2)+timeStep*y6);
        blue_goalkeeper.position = [left_goalkeeper_pos(1), left_goalkeeper_pos(2)+timeStep*y6];
    
    
        if blue_striker.position(1)> 5.5
            set(blue_strikerPos, 'XData', rightStriker_pos(1)-timeStep*1, 'YData',4);
            blue_striker.position = [rightStriker_pos(1)-timeStep*1, leftStriker_pos(2)];
        end
    
        if red_striker.position(1) <= 5.5 && blue_striker.position(1) > 3
            set(ballPos, 'XData', ball_pos(1)-timeStep*1, 'YData',4)
            set(red_strikerPos, 'XData', leftStriker_pos(1)-timeStep*1, 'YData',4);
            ball.position = [ball_pos(1)-timeStep*1, 4];
            red_striker.position = [leftStriker_pos(1)-timeStep*1, leftStriker_pos(2)];
